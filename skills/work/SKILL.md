...
**1. Read CLAUDE.md**
Understand the project purpose, your role, the stack, conventions, and GitLab coordinates.

**2. Read all specs**
List and read every file in `docs/specs/`. Understand what is specified, what each spec covers, and how specs relate to each other.
...
Confirm `GITLAB_TOKEN` is set. Confirm you are on `main` and it is up to date: `git pull origin main`.

Repeat the following until a stop condition is met.

### Step 0 — Re-read this skill

Read `skills/work/SKILL.md` in full before each iteration.
...
{ "assignee_id": <your user ID>, "labels": "in-progress" }
```
...
Read the issue title, description, and all comments. If the issue references a spec in `docs/specs/`, read it.
...
Check CI status. If still running, check again before proceeding to Step 9. If CI passes: proceed. If CI fails: fetch the job log, fix the failure, push again. If you cannot fix it after two attempts, stop and tell the user what is failing and why.
...
| MAJOR release required | "This is a MAJOR release because [reason]. Confirm?" |
