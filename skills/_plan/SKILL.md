...
- All files in `docs/decisions/` — decisions already made
...
Keep it brief. The user does not need to review or approve the backlog changes — those are the agent's job. They only need to know if something requires a product decision or milestone discussion.
